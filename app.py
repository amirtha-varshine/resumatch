...
def score_job(user_skills, resume, job_text):
    ...
    rs = set(tok(resume))
    jt = tok(job_text)
    kw = min(sum(1 for w in jt if w in rs) / len(jt) * 3, 1) if jt else 0
...
def ai_auto_find_jobs(skills, role, country, city):
    ...
    for i, sen in enumerate(SENIORITY):
        ...
        user_has_role_skills = [s for s in role_skills if s in skills]
        if len(user_has_role_skills) >= 3:
            needed = user_has_role_skills[:min(6, len(user_has_role_skills))]
        else:
            extra = [s for s in role_skills if s not in user_has_role_skills]
            needed = (user_has_role_skills + extra)[:5]
        ...
        scored = score_job(skills, "", j_text)
...
def analyze():
    ...
    if role and role in ROLE_SKILLS:
        ...
        if country_obj:
            # ROLE_SKILLS salary keys use "UK" but COUNTRIES_DATA uses ISO code "GB"
            code = country_obj["code"]
            salary_key = "UK" if code == "GB" else code
            salary_ref = ROLE_SKILLS[role]["salary"].get(salary_key)
